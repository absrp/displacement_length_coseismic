...
%%
%%%%%%%%%%%%%%%%%%%%%%% find nearest crack to each displacement measurement

    % measure distances between every crack and every displacement
    % measurement and measure the length of that crack
    % 
    ...
    % find minimum distance for each displacement measurement
    [dist,ind] = min(distance,[],2); 
    ID = ID(ind);
    L = L(ind);

    % Find unique values of ind and their corresponding indices
    [unique_ind, ~, ind_indices] = unique(ind,'stable');

    % Use accumarray to find the maximum slip value for each unique ind
    max_slip_values = accumarray(ind_indices, slip, [], @max);
    % Find the indices where max_slip_values occur
    max_slip_indices = accumarray(ind_indices, (1:numel(slip))', [], @(x) {x(x == max(x))});
    % Convert cell array to a numeric array
    max_slip_indices = cell2mat(max_slip_indices);

    L_maxdisp = L(max_slip_indices); 
    IDselect = ID(max_slip_indices);
    ...
    % subplot(2,3,i)
    % scatter(L_maxdisp, max_slip_values,'MarkerFaceColor',c,'MarkerEdgeColor','none','MarkerFaceAlpha',0.2)
    % hold on
...
% end
% 
% scatter(coordsx(max_slip_indices),coordsy(max_slip_indices),30,max_slip_values,'filled')
% colormap spring
% colorbar

%%%%%%%%%%%%%%%%%%%%%%%%%%%% relax nearest neighbor assumption, allow longest fracture within 10 m 
    % initiate displacement and length arrays
    distance = zeros(length(coords_ref),length(lines_secondary)); 
    sliptest = repmat(slip,1,length(lines_secondary));
    ID = [];
    % every column should be the same, different rows
    L = [];
    ...
    coordsxtest = repmat(coordsx,1,length(lines_secondary));
    coordsytest = repmat(coordsy,1,length(lines_secondary));
...
% Iterate over each row of distance
for row = 1:size(distance, 1)
    % Use logical indexing to find the non-zero elements in the row
    ...
    if ~isempty(nonzero_indices)
        ...
        coordsxL(row) = coordsxtest(row, idxL);
        coordsyL(row) = coordsytest(row, idxL);
    end
end
        select_ID = find(ID_L~=0);
